...
apiRouter.post('/', async (request, response) => {
  ...
  if (type === 'login') {
    ...
    res = await postReq(baseUsers, data)
      ...
      if(isMobile(request.headers['user-agent']) === true){
        res = 'You can\'t rate from a mobile'
      }else{
        const config = {
          headers: { Authorization:  request.headers.authorization },
        }
        res = await postReq(baseRatings, body, config)
      }
  }
  ...
  if (res) {
    if (res.length === 1) {
      ...
      response.status(Number(res[0])).send(res[1]).end()
    }
  }
})
